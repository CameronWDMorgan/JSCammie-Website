...
document.getElementById('generateButton').addEventListener('click', async function(event) {
    ...
    function getAllSelectedLoras() {
        const loraDataKeys = ['style', 'effect', 'concept', 'clothing', 'character', 'pose', 'background'];
        let combinedLora = [];
    
        loraDataKeys.forEach(category => {
        // Get the container for the current category
        const loraCategoryContainer = document.getElementById(category + '-loras');
    
        // Get all checked checkboxes within this category
        const selectedOptions = Array.from(loraCategoryContainer.querySelectorAll('input[type="checkbox"]:checked'))
            .map(checkbox => checkbox.value); // Assuming the value holds the id like 'style-pixelart'
    
        // Concatenate the selected options to the combinedLora array
        combinedLora = combinedLora.concat(selectedOptions);
        });
    
        console.log(combinedLora);
        return combinedLora; // Returns the array of selected lora IDs
    }
    
    // Example usage
    const selectedLoras = getAllSelectedLoras();
    ...
    selectedLoras.forEach(lora => {
        // add the lora strengths to lora_strengths:
        let loraStrength = document.getElementById('strength' + `-${lora}`).value
        lora_strengths.push(Number(loraStrength))
    })
